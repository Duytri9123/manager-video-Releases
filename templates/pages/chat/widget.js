...
(function () {
  ...
  async function loadModels(){
    ...
    for (const owner of ownerKeys) {
      ...
      for (const m of groups.get(owner)) {
        ...
        o.value = m.id; o.textContent = emoji + m.id;
        og.appendChild(o);
      }
      sel.appendChild(og);
    }
  ...
  async function sendNonStream(payload, holder){
    ...
    if (data.model && holder.tag){
      ...
      span.textContent = ' · ' + getEmoji(data.model) + data.model;
      holder.tag.appendChild(span);
    }
  ...
  async function send(){
    ...
    state.sending = true;
    const sendBtn = $('cw-send'), stopBtn = $('cw-stop'), inpEl = $('cw-input');
    ...
    try {
      ...
      console.error('[chat] error during send:', err);
      setBusy(false);
      state.abortCtl = null;
    }
  }
