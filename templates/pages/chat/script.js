...
(function () {
  ...
  async function chatRefreshStatus() {
    ...
    const headerRunBtn = document.getElementById('chat-header-btn-run');
    
    const mitmBadge = document.getElementById('chat-mitm-status');
    const mitmDesc = document.getElementById('chat-mitm-desc');
    const mitmToggleBtn = document.getElementById('chat-btn-mitm-toggle');
    const mitmTrustBtn = document.getElementById('chat-btn-mitm-trust');

    if (!data || !data.reachable) {
      _setBadge('9Router offline', 'red');
      if (headerRunBtn) headerRunBtn.style.display = 'inline-block';
      if (mitmBadge) mitmBadge.style.display = 'none';
      if (mitmDesc) mitmDesc.textContent = '9Router offline — không thể kiểm tra MITM.';
      if (mitmToggleBtn) {
        mitmToggleBtn.disabled = true;
        mitmToggleBtn.className = 'btn btn-secondary btn-sm';
        mitmToggleBtn.textContent = 'Bật MITM';
      }
      if (mitmTrustBtn) mitmTrustBtn.style.display = 'none';
      if (banner) {
        banner.innerHTML = '⚠ Không kết nối được 9Router tại <b>' + (data?.endpoint || 'localhost:20128') + '</b>. ' +
          '<button class="btn btn-warning btn-xs px-2 py-0.5 ml-2 font-bold inline-flex items-center gap-1" onclick="chatStart9Router()">' +
          '⚡ Cài đặt / Khởi chạy 9Router (Admin)' +
          '</button>';
      }
      if (autoBtn) autoBtn.disabled = true;
      return data;
    }
    if (headerRunBtn) headerRunBtn.style.display = 'none';
    
    // Update MITM UI when online
    if (data.mitm) {
      const running = !!data.mitm.running;
      const trusted = !!data.mitm.certTrusted;
      
      if (mitmBadge) {
        mitmBadge.style.display = running ? 'inline-block' : 'none';
      }
      
      if (mitmDesc) {
        if (running) {
          if (!trusted) {
            mitmDesc.innerHTML = '<span class="text-orange-500 font-semibold">⚠ Đang chạy · Chứng chỉ chưa tin cậy (bấm Tin cậy Cert).</span>';
          } else {
            mitmDesc.innerHTML = '<span class="text-emerald-500 font-semibold">✓ Đang hoạt động (đã chuyển hướng API).</span>';
          }
        } else {
          mitmDesc.textContent = 'Chưa chạy. Lắng nghe trên cổng 443 để chặn/chuyển hướng API.';
        }
      }
      
      if (mitmToggleBtn) {
        mitmToggleBtn.disabled = false;
        if (running) {
          mitmToggleBtn.className = 'btn btn-danger btn-sm';
          mitmToggleBtn.textContent = '🛑 Dừng MITM';
        } else {
          mitmToggleBtn.className = 'btn btn-success btn-sm';
          mitmToggleBtn.textContent = '🚀 Bật MITM';
        }
      }
      
      if (mitmTrustBtn) {
        mitmTrustBtn.style.display = (running && !trusted) ? 'inline-block' : 'none';
      }
    }
  ...
  async function chatStart9Router() {
    _toast('Đang gửi lệnh cài đặt / khởi chạy 9Router...', 'info');
    const { data, ok } = await _post('/api/chatbot/start_9router');
    if (!ok || data?.ok === false) {
      _toast('Lỗi: ' + (data?.message || 'Không thể gửi yêu cầu khởi chạy.'), 'error');
      return;
    }
    _toast(data.message || 'Yêu cầu khởi chạy thành công. Vui lòng kiểm tra màn hình của bạn.', 'success');
    // Poll status a few times after starting
    setTimeout(chatRefreshStatus, 2000);
    setTimeout(chatRefreshStatus, 6000);
    setTimeout(chatRefreshStatus, 12000);
  }

  async function chatMitmToggle() {
    if (!state.status) await chatRefreshStatus();
    const running = !!state.status?.mitm?.running;
    const action = running ? 'stop' : 'start';
    
    _toast((running ? 'Đang dừng' : 'Đang khởi chạy') + ' MITM Proxy...', 'info');
    const { data, ok } = await _post('/api/chatbot/mitm', { action });
    if (!ok || data?.ok === false) {
      _toast('Lỗi: ' + (data?.message || 'Thao tác MITM thất bại.'), 'error');
      return;
    }
    _toast(running ? 'Đã dừng MITM Server.' : 'Đã khởi chạy MITM Server thành công.', 'success');
    await chatRefreshStatus();
  }

  async function chatMitmAction(action) {
    _toast('Đang thực hiện thao tác chứng chỉ...', 'info');
    const { data, ok } = await _post('/api/chatbot/mitm', { action });
    if (!ok || data?.ok === false) {
      _toast('Lỗi: ' + (data?.message || 'Thao tác chứng chỉ thất bại.'), 'error');
      return;
    }
    _toast('Thao tác chứng chỉ thành công.', 'success');
    await chatRefreshStatus();
  }
  ...
  async function chatLoadModels() {
    ...
    const renderInto = (el, includeBlank) => {
      ...
      for (const [owner, items] of groups) {
        ...
        for (const m of items) {
          const opt = document.createElement('option');
          const emoji = { cx:'⚡ ', kr:'🥝 ', gemini:'🔷 ', gc:'🐙 ', ag:'🌌 ', combo:'✨ ' }[owner] || '🤖 ';
          opt.value = m.id;
          opt.textContent = emoji + m.id;
          og.appendChild(opt);
        }
        el.appendChild(og);
      }
  ...
  async function _sendStream(payload, modelLabel) {
    ...
    if (usage && placeholder.meta) {
      ...
      const getEmoji = (m) => {
        ...
        if (ml.includes('gemini')) return '🔷 ';
        if (ml.includes('gpt') || ml.includes('openai') || ml.includes('claude')) return '⚡ ';
        if (ml.includes('llama') || ml.includes('groq')) return '🦙 ';
        if (ml.includes('qwen')) return '🥝 ';
        if (ml.includes('deepseek')) return '🐳 ';
        return '🤖 ';
      };
      span.textContent = ` · ${usage.total_tokens || 0} tokens (in ${usage.prompt_tokens || 0} / out ${usage.completion_tokens || 0})${finishReason ? ' · ' + finishReason : ''} · ${getEmoji(actualModel)}${actualModel}`;
      placeholder.meta.appendChild(span);
  ...
  async function _sendNonStream(payload, modelLabel) {
    ...
    if (data.usage && placeholder.meta) {
      ...
      const modelName = data.model || modelLabel;
      span.textContent = ` · ${u.total_tokens || 0} tokens (in ${u.prompt_tokens||0} / out ${u.completion_tokens||0}) · ${getEmoji(modelName)}${modelName}`;
      placeholder.meta.appendChild(span);
    }
  ...
  async function chatSend() {
    ...
    // Synchronous lock first to prevent double-submits
    state.sending = true;
    const btnSend = document.getElementById('chat-send-btn');
    const btnStop = document.getElementById('chat-stop-btn');
    ...
    try {
      ...
      state.sending = false;
      if (btnSend) { btnSend.disabled = false; btnSend.textContent = 'Gửi'; }
    ...
    try {
      ...
      console.error('[chat] error during chatSend:', err);
      _setSendBusy(false);
      state.abortCtl = null;
    }
  }
  ...
  window.chatMitmAction   = chatMitmAction;
