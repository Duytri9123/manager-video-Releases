...
window.aiLoadConfig = async function() {
  try {
    ...
    // Filter model selects based on active keys
    const activeProviders = d.llm_providers || [];
    const filterSelect = (selectId, providerCheckFn) => {
      ...
      if (!el._origOptions) {
        ...
        Array.from(el.children).forEach(child => {
          if (child.tagName === 'OPTGROUP') {
            const grpOpts = Array.from(child.children).map(opt => ({
              value: opt.value,
      ...
      const filtered = el._origOptions.filter(opt => providerCheckFn(opt.value));
      
      el.innerHTML = '';
      const groups = {};
      filtered.forEach(opt => {
        if (opt.optgroupLabel) {
          if (!groups[opt.optgroupLabel]) {
            const grp = document.createElement('optgroup');
          ...
          groups[opt.optgroupLabel].appendChild(optionEl);
        } else {
          const optionEl = document.createElement('option');
          optionEl.value = opt.value;
          optionEl.textContent = opt.text;
      ...
      if (el.value !== currentVal && el.options.length) {
