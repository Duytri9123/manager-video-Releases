...
(function () {
  ...
  async function refreshStatus() {
    ...
    try {
      ...
      const mvSourceSelect = document.getElementById('mv-source');
      if (mvSourceSelect) {
        if (!mvSourceSelect._origOptions) {
          mvSourceSelect._origOptions = Array.from(mvSourceSelect.options).map(opt => ({
            value: opt.value,
            text: opt.textContent,
            selected: opt.selected
          }));
        }
        const currentVal = mvSourceSelect.value;
        const activeOpts = mvSourceSelect._origOptions.filter(opt => {
          if (opt.value === 'tmdb') {
            return !!r.configured;
          }
          return true;
        });
        mvSourceSelect.innerHTML = '';
        activeOpts.forEach(opt => {
          ...
          mvSourceSelect.appendChild(optionEl);
        });
        if (mvSourceSelect.value !== currentVal && mvSourceSelect.options.length) {
          mvSourceSelect.options[0].selected = true;
        }
      }
