...
// Attach listener to init listeners on DOM load
...
// Nền tảng hiện tại của kết quả tìm kiếm (douyin | tiktok | youtube | ...)
let _userPlatform = 'douyin';

// Trả về URL thật của một video. Với Douyin thì build từ aweme_id; các nền
// tảng khác dùng URL do yt-dlp trả (v.url).
function _videoUrl(v) {
  if (v && v.url && /^https?:\/\//i.test(v.url)) return v.url;
  return 'https://www.douyin.com/video/' + (v?.aweme_id || '');
}

const _PLATFORM_META = {
  douyin:    { icon: '🎵', label: 'Douyin' },
  tiktok:    { icon: '🎬', label: 'TikTok' },
  youtube:   { icon: '▶️', label: 'YouTube' },
  instagram: { icon: '📸', label: 'Instagram' },
  facebook:  { icon: '📘', label: 'Facebook' },
  bilibili:  { icon: '📺', label: 'Bilibili' },
  kuaishou:  { icon: '⚡', label: 'Kuaishou' },
  twitter:   { icon: '🐦', label: 'X / Twitter' },
  unknown:   { icon: '🌐', label: 'Khác' },
};

// Suy đoán nền tảng ở phía client để hiển thị badge trước khi tìm.
...
function _updatePlatformBadge(platform) {
  const el = document.getElementById('platform-badge');
  if (!el) return;
  const meta = _PLATFORM_META[platform] || _PLATFORM_META.unknown;
  el.innerHTML = '<span>' + meta.icon + '</span><span>' + meta.label + '</span>';
  el.className = 'platform-badge pf-' + platform;
}

// Thumbnail: cover Douyin/TikTok CDN đi qua proxy (tránh CORS/referer),
// còn lại (YouTube, ...) load trực tiếp.
function _thumbUrl(v) {
  const cover = v?.cover || '';
  if (!cover) return '';
  if (/douyinpic|byteimg|tiktokcdn|douyin|pstatp/i.test(cover)) {
    return '/api/proxy_image?url=' + encodeURIComponent(cover);
  }
  return cover;
}
...
async function _ensureItemsTranslatedForQueue(items) {
  ...
  let provider = document.getElementById('provider-select')?.value || 'auto';
  if (provider === 'prompt_connect') provider = 'auto';
...
async function goNext() {
  ...
  if (_userHasMore) {
    ...
    _renderVideos();
    if (_viEnabled) _translateVisibleDebounced(1200);
  }
}
...
// Tìm user đa nền tảng theo kiểu streaming: phát video dần khi server lấy được.
async function _searchUserStream(url, currentProvider) {
  ...
  if (btnLoadAll) { btnLoadAll.classList.add('hidden'); btnLoadAll.style.display = 'none'; }
  ...
  const scheduleRender = () => {
    ...
    renderTimer = setTimeout(() => {
      renderTimer = null;
      _renderVideos();
      if (_viEnabled) _translateVisibleDebounced(600);
    }, 250);
  };

  let gotError = '';
  try {
    const response = await fetch('/api/user_stream', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (chunk.kind === 'profile') {
          _userPlatform = chunk.platform || _guessPlatform(url);
          _userSecUid = chunk.sec_uid || '';
          _userAwemeCount = parseInt(chunk.aweme_count) || 0;
          _updatePlatformBadge(_userPlatform);

          const avatarEl = document.getElementById('u-avatar');
          if (avatarEl) {
            avatarEl.innerHTML = chunk.avatar
              ? '<img class="user-avatar" src="' + _thumbUrl({ cover: chunk.avatar }) + '">'
              : '<div class="user-avatar-ph">👤</div>';
          }
          const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val || ''; };
          set('u-name', chunk.nickname);
          set('u-sig', chunk.signature);
          set('u-posts', chunk.aweme_count ? fmtNum(chunk.aweme_count) : '—');
          set('u-followers', chunk.follower ? fmtNum(chunk.follower) : '—');
          set('u-following', chunk.following ? fmtNum(chunk.following) : '—');
        ...
        } else if (chunk.kind === 'videos' && Array.isArray(chunk.videos)) {
          for (const v of chunk.videos) {
            const key = String(v.aweme_id || v.url || '');
            if (!key || seen.has(key)) continue;
            seen.add(key);
            _userVideos.push(v);
          }
          if (statusEl) {
            statusEl.textContent = _userVideos.length + ' video'
              + (_userAwemeCount ? ' / ' + _userAwemeCount : '') + ' (đang tải...)';
            statusEl.style.color = '';
          }
          scheduleRender();
        } else if (chunk.kind === 'progress') {
          if (statusEl && chunk.collected !== undefined) {
            statusEl.textContent = 'Đã tìm ' + chunk.collected + ' video...';
          }
        } else if (chunk.kind === 'error') {
          gotError = chunk.message || 'Lỗi không xác định';
        } else if (chunk.kind === 'done') {
          if (statusEl) statusEl.textContent = (chunk.total ?? _userVideos.length) + ' video';
        }
      }
    }
  } catch (e) {
    ...
    _renderVideos();
    if (_viEnabled) _translateVisibleDebounced(400);
  }

  if (gotError && !_userVideos.length) {
    toast(gotError, 'error');
    if (statusEl) statusEl.textContent = '';
  } else if (gotError) {
    toast(gotError, 'warning');
  }
}
...
async function searchUser() {
  ...
  let currentProvider = document.getElementById('provider-select')?.value || 'auto';
  if (currentProvider === 'prompt_connect') currentProvider = 'auto';
  _setTranslationContext(url, currentProvider);
  _setProfileTranslationContext(url, currentProvider);

  // Nền tảng khác Douyin → hiển thị video dần (streaming) thay vì đợi tải hết.
  if (_guessPlatform(url) !== 'douyin') {
    return _searchUserStream(url, currentProvider);
  }
  ...
  try {
    ...
    _userPlatform = info.platform || 'douyin';
    _updatePlatformBadge(_userPlatform);
...
function _initUserPageListeners() {
  ...
  if (urlInput) {
    const syncBadge = () => _updatePlatformBadge(_guessPlatform(urlInput.value.trim()));
    urlInput.addEventListener('input', syncBadge);
    urlInput.addEventListener('keydown', e => { if (e.key === 'Enter') searchUser(); });
    syncBadge();
  }
...
async function _loadTranslationStatus() {
  try {
    ...
    if (typeof window.filterActiveProviders === 'function') {
      status = await window.filterActiveProviders();
    }
    if (!status) {
