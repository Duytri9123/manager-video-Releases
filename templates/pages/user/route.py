...
def _cookiefile_for(platform: str):
    """Trả về đường dẫn cookie file (nếu có) cho nền tảng, giúp yt-dlp lấy được
    nội dung cần đăng nhập/khu vực giới hạn."""
    import os
    candidates = {
        "tiktok":   [".cookies.json", "cookies_tiktok.txt", "tiktok_cookies.txt"],
        "youtube":  ["cookies_youtube.txt", "youtube_cookies.txt", "cookies.txt"],
        "instagram": ["cookies_instagram.txt", "instagram_cookies.txt"],
        "facebook": ["cookies_facebook.txt", "facebook_cookies.txt"],
    }
    for name in candidates.get(platform, []):
        if os.path.exists(name):
            # yt-dlp chỉ đọc cookie dạng Netscape .txt; bỏ qua .json
            if name.endswith(".txt"):
                return name
    return None


# ── /api/user_stream — phát video dần (NDJSON) cho nền tảng khác Douyin ──────
@bp.route("/api/user_stream", methods=["POST"])
def user_stream():
    ...
    from core.multi_platform import detect_platform, stream_profile
    platform = detect_platform(url)
    max_videos = int(data.get("max_videos", 0) or 0)
    ...
    try:
        ...
        browser_opts = dict((config.get("browser_fallback") or {})) if hasattr(config, "get") else {}
    except Exception:
        proxy = None

    cookiefile = _cookiefile_for(platform)

    import json as _j

    def generate():
        try:
            for chunk in stream_profile(url, max_videos=max_videos,
                                        cookiefile=cookiefile, proxy=proxy,
                                        browser_opts=browser_opts):
                yield _j.dumps(chunk, ensure_ascii=False) + "\n"
        except Exception as e:  # noqa: BLE001
            yield _j.dumps({"kind": "error", "message": str(e)}, ensure_ascii=False) + "\n"
...
def user_videos_page():
    ...
    # Nền tảng khác Douyin: dữ liệu đã được /api/user_info trả đủ trong 1 lần
    # (yt-dlp), nên không cần phân trang thêm.
    from core.multi_platform import is_douyin
    if not is_douyin(url):
        return jsonify({"videos": [], "has_more": False, "next_cursor": 0, "offset": offset})
...
def proxy_image():
    ...
    allowed = ("douyinpic.com", "byteimg.com", "tiktokcdn.com",
               ...
               "tiktokcdn-us.com", "ttwstatic.com", "muscdn.com",
               "ytimg.com", "ggpht.com", "cdninstagram.com", "fbcdn.net")
...
def user_info():
    ...
    # ── Nền tảng khác Douyin (TikTok/YouTube/...) → dùng yt-dlp ──────────────
    from core.multi_platform import is_douyin, detect_platform, fetch_profile
    if not is_douyin(url):
        platform = detect_platform(url)
        proxy = None
        browser_opts = {}
        ...
        try:
            ...
            return jsonify({"error": f"{platform}: {e}"}), 500
